...
import type { IInfo, ISocketInfo, IStreamDict } from "./model";
...
class RedisCommandHandler {
  ...
  private STORED_STREAMS: IStreamDict = {};
    ...
    for (let i = 0; i < decodedData.length; i++) {
      ...
      switch (decodedData[i][0].toLowerCase()) {
        ...
        case "xadd":
          ...
          this.STORED_STREAMS[streamKey].push([
            streamID,
        ...
        case "xrange":
          {
            ...
            this.STORED_STREAMS[streamKey].forEach(([streamEntryID, _], i) => {
              if (
            ...
            responseArr = this.STORED_STREAMS[streamKey].slice(
              startingIndex,
              endingIndex
                ? endingIndex + 1
                : this.STORED_STREAMS[streamKey].length
            );
  ...
  private parseStreamID(streamID: string, streamKey: string): number[] {
    try {
      ...
      if (val2 === "*") {
        const topStreamEntry = this.STORED_STREAMS[streamKey]?.slice().pop();
        let topSeqNum = 0;
        let topMs = 0;
  ...
  private parseMsAndSeqNum(streamID: string) {
